...
import 'package:starlightserviceapp/models/app/rack/rack-getbycode-model.dart';
...
class RackService {
  Future<List<RackModel>> getById(BuildContext context, String rackCode) async {
    StorageApp storageApp = StorageApp();
    AppService appService = AppService();
    try {
      String token = await storageApp.getAccessToken();
      final url =
          Uri.https(URL_API_APP, "$VERSION_API_APP/msrack/getbycode/$rackCode");
      final headers = {
      ...
      if (appService.checkExpired(response.statusCode)) {
        ...
        if (res != null) {
          return getById(context, rackCode);
        } else {
          throw ("Token Expired");
        }
      } else if (response.statusCode == 200) {
        GetByCodeModel data =
            GetByCodeModel.fromJson(jsonDecode(response.body));
        if (data.code == 200) {
