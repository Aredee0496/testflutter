...
import 'package:starlightserviceapp/pages/tank-driver/tank-driver-details.dart';
...
class TankDriverPage extends StatefulWidget {
  const TankDriverPage({super.key});

  @override
  State<TankDriverPage> createState() => _TankDriverPageState();
}

class _TankDriverPageState extends State<TankDriverPage> {
  ...
  List<TrTankManagementsRow> items = [];
  List<TrTankManagementsRow> itemsShow = [];
  ...
  Future<void> getStatus() async {
    try {
      ...
      itemsStatus = [];
      List<JobStatusModel> jobStatus = await tankService.getJobStatus(context);

      for (var i = 0; i < jobStatus.length; i++) {
        JobStatusModel item = jobStatus[i];
        setState(() {
  ...
  fetchData() async {
    try {
      ...
      List<TrTankManagementsRow> data =
  ...
  toPageDetail(
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
          builder: (context) => TankDriverDetailsPage(
                id: '${data?["id"] ?? 0}',
                jobStatus: jobStatus,
              )),
    ).then((_) => setState(() {
  ...
  onAdd() {
    Navigator.push(
      context,
      MaterialPageRoute(
          builder: (context) => TankDriverEditPage(
                action: "add",
              )),
    ).then((_) => setState(() {
          fetchData();
  ...
  Widget buildPosts(List<TrTankManagementsRow> data) {
