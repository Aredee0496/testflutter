...
import 'package:starlightserviceapp/pages/transportclearing/transportclearing-details.dart';
...
class TransportClearingPage extends StatefulWidget {
  const TransportClearingPage({super.key});

  @override
  State<TransportClearingPage> createState() => _TransportClearingPageState();
}

class _TransportClearingPageState extends State<TransportClearingPage> {
  ...
  List<TrTransportClearingSearchRow> items = [];
  List<TrTransportClearingSearchRow> itemsShow = [];
  ...
  fetchData() async {
    try {
      ...
      List<TrTransportClearingSearchRow> data =
          await transportClearingService.search(context, payload);
      setState(() {
        items = data;
        itemsShow = data;
      });
  ...
  toPageDetail(
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
          builder: (context) => TransportClearingDetailsPage(
                id: '${data?["id"] ?? 0}',
                jobStatus: jobStatus,
              )),
    ).then((_) => setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: MyAppBar(
          title: "รายการเบิกค่าใช้จ่ายการเดินทาง",
          listWidget: [
            IconButton(
  ...
  Widget buildPosts(List<TrTransportClearingSearchRow> data) {
